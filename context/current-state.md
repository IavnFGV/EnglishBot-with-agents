...
- Learner training now has an optional internal-only TTS path: when `ENGLISHBOT_TTS_BASE_URL` is configured, active training cards get a localized `Listen` button that sends only the current English answer text to the separate TTS service and delivers the result as Telegram `voice`.
- TTS stays fail-closed and optional: startup does not health-gate on the external service, quiz flow keeps working when TTS is disabled or unavailable, `/settings` can persist an optional learner `tts_voice_id`, and missing/removed voices fall back to the TTS service default voice.
...
- Telegram `file_id` reuse is now an explicit cache-only persistence layer keyed by asset and media kind (`photo` today, `audio`/`voice` ready for future TTS-backed flows); it accelerates delivery but does not replace local files as runtime truth.
...
- `chain_of_commands/059-persisted-tts-audio-variants-and-telegram-cache.md` now captures the follow-up TTS storage slice: let one `learning_item` own multiple local synthesized audio variants distinguished by composed identity such as `learning_item` + `voice_id` + model/engine key, while keeping Telegram `file_id` reuse as a separate transport cache above those persisted variants.
