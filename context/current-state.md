...
- On deployed servers, workbook export now prefers the current `INFRA_STATIC_BASE_URL` for that display-only `image` formula when the linked image points at a local runtime asset under `/app/assets`, so exported sheets preview the bot's current nginx-served static file instead of the original remote download URL; the original asset `source_url` still remains only traceability metadata in SQLite.
