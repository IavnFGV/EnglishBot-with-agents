...
- Static learner question images use a separate Telegram transport cache: both first-time sends and successful in-place `editMessageMedia` uploads persist returned `photo` `file_id`s in SQLite, repeat renders prefer that cache, and stale Telegram ids fail closed by re-uploading the local file and refreshing the cached id.
...
- Workbook asset preparation failures now surface the exact spreadsheet row, word, asset field, offending value, and original reason in the Telegram response while leaving SQLite unchanged.
...
- The active registered command list is now family-first plus build and owner bootstrap support: `/start`, `/help`, `/version`, `/learn`, `/homework`, `/me`, `/settings`, `/cancel`, `/create_assignment`, `/seed_demo`, `/topics`, `/teacher_content`, and `/bulk_edit`; `/version` reports the deployed build metadata already used by startup/status reporting.
...
- The Docker Compose app service reads both `/srv/services/englishbot/infra-runtime.env` for infra-managed URLs and `/srv/services/englishbot/build.env` for non-secret build metadata; GitHub Actions atomically refreshes `build.env` on deploy so later manual container recreates preserve `/version` values instead of falling back to `dev`/`unknown`.
