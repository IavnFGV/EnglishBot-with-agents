...
- Large `/bulk_edit` applies now keep the reused control message alive with an explicit in-progress status while the workbook import runs, and the Apply callback is acknowledged immediately so long imports with remote image downloads no longer look hung or hit Telegram callback expiry.
- That in-progress `/bulk_edit` status is now row-aware too: during apply the control message shows processed rows versus total rows, not just a generic spinner, so long family imports can be watched live from Telegram.
