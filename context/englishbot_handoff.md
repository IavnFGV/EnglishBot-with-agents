...
## 3.5 Teacher–Student модель как текущий MVP flow

### Teacher–Student связь

Текущий runtime проекта поддерживает явную модель связи между учителем и учениками.

Это важно как действующий MVP flow, но больше не должно считаться долгосрочной моделью владения. Новая продуктовая основа смещается в сторону `workspace`-владения, а миграция существующих teacher-student flow в этот слой выполняется поэтапно.
...
- пользователь без учителя может:
  ...
  - работать без отдельного teacher-link, не требуя, чтобы система считала его “сам себе учителем” как долгосрочную модель владения
...
Дополнение: поддержка нескольких языков

Начиная с текущей версии, `learning_item` больше не ограничен одним переводом.

Каждый `learning_item` может иметь несколько языковых представлений, хранящихся в `learning_item_translations`.

Это позволяет:

- хранить переводы на нескольких языках (например: ru, uk, bg и т.д.);
- использовать разные языки как вход для вопроса (например: "липень" → "July");
- учитывать реальные многоязычные сценарии обучения (домашний язык + язык школы + английский).

При этом сохраняется текущая архитектурная модель:

- `lexeme` остаётся носителем основного слова (обычно английского);
- `learning_item` остаётся основной единицей обучения;
- `learning_item_translations` расширяет learning_item, не ломая существующие связи.

Дополнительный инвариант:

- один `learning_item` может иметь несколько переводов на разные языки;
- для каждого языка предполагается один основной (primary) перевод в рамках MVP;
- проверка ответов должна опираться либо на `lexeme.headword`, либо на соответствующий перевод из `learning_item_translations`, в зависимости от сценария вопроса.
...
## 4.1 Workspaces и владение контентом

Система должна поддерживать два типа workspace:

### Типы workspace

#### Teacher workspace (workspace автора)

Используется для:

- создания и редактирования контента;
- хранения каталога слов;
- подготовки тем (topics) и обучающих единиц (learning_items).

Характеристики:

- принадлежит одному учителю или группе учителей;
- только участники с ролью `teacher` могут редактировать контент;
- содержит:
  - `learning_items`
...
#### Student workspace (workspace обучения)

Используется для:

- прохождения обучения учеником;
- использования опубликованного учебного контента;
- хранения прогресса;
- выполнения домашних заданий;
- совместной работы нескольких взрослых с учеником.

Характеристики:

- содержит одного или нескольких учеников;
- может содержать нескольких учителей (например, родители);
- является основным пространством обучения;
- содержит:
  - доступный ученику опубликованный контент
  - assignments
  - доступные темы (topic access)
  - прогресс
  - training sessions
...
Участники задаются через `workspace_members`:

- `teacher`:
  - редактирует контент (в teacher workspace);
  - назначает задания;
  - управляет доступом к темам;
  - просматривает прогресс;

- `student`:
  - проходит задания;
  - видит доступные темы;
  - не редактирует контент;

---

## 4.2 Владение учебным контентом

Контент не принадлежит пользователю напрямую.

Правило:

- `learning_items`, `learning_item_translations`, `topics` принадлежат **workspace**.

---

### Источник контента

- контент создаётся в **teacher workspace**;
- затем добавляется / публикуется в **student workspace** для использования в обучении.

---

## 4.3 Перенос контента между workspace

Контент не используется напрямую между workspace.

Используется операция:

> **добавление / публикация (publish)**

### Поведение

- учитель выбирает topic или `learning_items` из teacher workspace;
- система добавляет их в student workspace;
- после этого student workspace использует контент независимо.

---

### Инвариант

- student workspace не должен зависеть от “живых” ссылок на teacher workspace;
- после добавления контент считается локальным для student workspace (логически).

---

## 4.4 Политика редактирования контента

Редактирование контента допускается.

- только участники с ролью `teacher` могут редактировать контент в teacher workspace;
- student workspace не является authoring-space;

### Разрешено:

- исправление опечаток;
- изменение переводов;
- переименование topics;
- добавление `learning_items` в topic;
- удаление `learning_items` из topic (для будущих запусков);

---

### Ограничение: активные сессии

- уже начатая training session не должна изменяться;
- список `learning_item_id` фиксируется при старте;

---

## 4.5 Политика удаления

Физическое удаление запрещено для используемых сущностей.
...
- `learning_items`, если они:
  - используются в assignment;
  - участвуют в training session;

- `topics`, если они:
  - назначены ученику;
  - используются в assignment;
  - доступны через topic access;

---

### Поведение archived сущностей:

- не используются в новых назначениях;
- могут существовать в старых assignment;
- могут быть скрыты или помечены как удалённые;

---

## 4.6 Assignment как снапшот

Assignment не должен зависеть от текущего состояния topic.

### При создании assignment:

- фиксируется список `learning_item_id`;
- сохраняется название (title);

---

### Следствия:

- изменения topic не влияют на уже выданные задания;
- удаление элементов из topic не ломает assignment;
- добавление элементов в topic не меняет assignment;

---

## 4.7 Разделение состояний

Система разделяет данные на два типа:
...
> Живые изменения не должны ломать уже выданные задания и активные сессии.
