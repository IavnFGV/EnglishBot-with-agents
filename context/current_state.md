...
- Repository now includes a minimal SQLite seeder for starter vocabulary packs; the current seeded packs are `weekdays` (7 items), `months` (6 items), and `colors` (6 items), each stored as ordinary `learning_items` with `ru`, `uk`, and `bg` translations and no new topic tables.
...
- Teachers can now use the same `/assign` command with a seeded group name as the primary UX path: `/assign <student_user_id> <group_name>` resolves starter packs (`weekdays`, `months`, `colors`) to SQLite `learning_item` ids, stores a human-readable assignment title, and still keeps the legacy raw-id path as fallback.
...
- Focused tests now live in `tests/test_user_profiles.py`, `tests/test_teacher_student.py`, `tests/test_teacher_handlers.py`, `tests/test_vocabulary.py`, `tests/test_training.py`, `tests/test_training_handlers.py`, `tests/test_homework.py`, `tests/test_homework_handlers.py`, and `tests/test_basic_topics_seed.py`.
- Current constraints stay intentionally narrow: no FSM, no deep links, no middleware changes, no topic or lesson selection for training, no answer history persistence, no statistics beyond per-session counters, no asset playback, and only one minimal fixed-set homework type over assigned `learning_item` ids whether assigned directly or via a seeded named group.
- Focused tests now also cover training service behavior and thin `/learn` command flow without depending on Telegram runtime.
- Changed files for the current slice: `englishbot/db.py`, `tests/test_user_profiles.py`, `CHANGELOG.md`, `context/current_state.md`.
