...
- SQLite schema is created automatically on startup and now bootstraps `users`, `user_profiles`, `interactions`, `invites`, `teacher_student_links`, `lexemes`, `learning_items`, `learning_item_translations`, `topics`, `topic_learning_items`, `training_sessions`, `training_session_items`, `assignments`, and `assignment_items`.
...
- Repository now includes a minimal SQLite seeder for starter vocabulary packs; the current seeded packs are `weekdays` (7 items), `months` (6 items), and `colors` (6 items), and startup seeds them as real persisted `topics` linked to ordinary `learning_items` with `ru`, `uk`, and `bg` translations.
...
- Teachers can now use the same `/assign` command with a topic name as the primary UX path: `/assign <student_user_id> <topic_name>` resolves persisted `topics.name` (`weekdays`, `months`, `colors`) to linked SQLite `learning_item` ids, stores the topic title as the human-readable assignment title, and still keeps the legacy raw-id path as fallback.
