...
def list_accessible_topics(student_user_id: int) -> list[sqlite3.Row]:
    ...
    if family is not None:
        return _list_family_topics(int(family["id"]))
    with get_connection() as connection:
        return connection.execute(
            """
            SELECT
...
def _list_family_topics(family_id: int) -> list[sqlite3.Row]:
    with get_connection() as connection:
        return connection.execute(
            """
            SELECT
                ...
                topics.title,
                topics.created_at,
                NULL AS granted_by_teacher_user_id,
                topics.created_at AS access_created_at,
                COUNT(learning_items.id) AS item_count
            FROM topics
...
def start_topic_training_session(
    ...
    if topic["family_id"] is not None:
        family = get_user_family(student_user_id)
        if family is None or int(topic["family_id"]) != int(family["id"]):
            raise TopicAccessDeniedError
